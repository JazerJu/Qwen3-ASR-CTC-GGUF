...
llama_log_set: Any = None
llama_backend_init: Any = None
llama_backend_free: Any = None
llama_model_default_params: Any = None
llama_model_load_from_file: Any = None
llama_model_free: Any = None
llama_model_get_vocab: Any = None
llama_context_default_params: Any = None
llama_init_from_model: Any = None
llama_free: Any = None
llama_batch_init: Any = None
llama_batch_free: Any = None
llama_decode: Any = None
llama_get_logits: Any = None
llama_get_logits_ith: Any = None
llama_get_embeddings: Any = None
llama_tokenize: Any = None
llama_vocab_n_tokens: Any = None
llama_vocab_eos: Any = None
llama_vocab_bos: Any = None
llama_token_to_piece: Any = None
llama_get_memory: Any = None
llama_memory_clear: Any = None
llama_model_n_embd: Any = None

# Sampler
llama_sampler_chain_default_params: Any = None
llama_sampler_chain_init: Any = None
llama_sampler_chain_add: Any = None
llama_sampler_init_greedy: Any = None
llama_sampler_init_dist: Any = None
llama_sampler_init_temp: Any = None
llama_sampler_init_top_k: Any = None
llama_sampler_init_top_p: Any = None
llama_sampler_sample: Any = None
llama_sampler_free: Any = None
llama_sampler_init_min_p: Any = None
llama_sampler_init_penalties: Any = None
llama_sampler_accept: Any = None
...
def bind_llama_lib():
    ...
    # Linux-only llama.cpp shared libraries. Keep absolute paths so imports work
    # regardless of the current working directory.
    lib_dir = "/data/推理框架/llama.cpp/build/bin"
    GGML_DLL = "libggml.so"
    ...
    ggml_base = ctypes.CDLL(os.path.join(lib_dir, GGML_BASE_DLL), mode=mode)
    ctypes.CDLL(os.path.join(lib_dir, "libggml-cpu.so"), mode=mode)
    cuda_lib = os.path.join(lib_dir, "libggml-cuda.so")
    if os.path.exists(cuda_lib):
        ctypes.CDLL(cuda_lib, mode=mode)
    ggml = ctypes.CDLL(os.path.join(lib_dir, GGML_DLL), mode=mode)
    llama = ctypes.CDLL(os.path.join(lib_dir, LLAMA_DLL), mode=mode)
...
def init():
    """初始化 Linux llama.cpp shared libraries without changing cwd."""
    lib_dir = "/data/推理框架/llama.cpp/build/bin"
    os.environ["PATH"] = lib_dir + os.pathsep + os.environ.get("PATH", "")
    os.environ["LD_LIBRARY_PATH"] = lib_dir + os.pathsep + os.environ.get("LD_LIBRARY_PATH", "")
    logger.info(f"初始化 llama.cpp shared libraries: {lib_dir}")
    bind_llama_lib()
    return True
...
class LlamaModel:
    """模型的面向对象封装"""
    def __init__(self, path, n_gpu_layers=-1, use_gpu=True):
    ...
    def load_model(self, model_path: str, n_gpu_layers: int = -1, use_gpu: bool = False):
        """
        ...
        model = llama_model_load_from_file(
            model_path_obj.as_posix().encode('utf-8'),
            model_params
        )
...
class LlamaBatch:
    ...
    def set_embd(self, data: Any, pos: Union[Any, int] = 0, seq_id: int = 0):
        """
...
def _skip_gguf_value(mm: Any, offs: int, v_type: int) -> int:
    buf: Any = mm
    ...
    if val_len > 0:
        ...
        slen = struct.unpack_from("<Q", buf, offs)[0]
        return offs + 8 + slen
...
def get_token_embeddings_gguf(model_path, target_tensor="token_embd.weight"):
    ...
    """
    t_start = time.time()
    mm: Any = np.memmap(model_path, mode='r')
    buf: Any = mm
    ...
    for _ in range(kv_count):
        key_len = struct.unpack_from("<Q", buf, offs)[0]
        offs += 8
        if key_len == 17 and mm[offs:offs+17].tobytes() == b'general.alignment':
            offs += 17
            v_type = struct.unpack_from("<I", buf, offs)[0]
            offs += 4
            if v_type == 4: # UINT32
                alignment = int(struct.unpack_from("<I", buf, offs)[0])
                offs += 4
                continue
        else:
            offs += key_len
    ...
    target_rel_offset = 0
    target_type = 0
    target_shape: Any = None # GGUF shape 是倒序的 [n_embd, vocab_size]
    
    target_bytes = target_tensor.encode('utf-8')
    for _ in range(tensor_count):
        name_len = struct.unpack_from("<Q", buf, offs)[0]
        offs += 8
        is_target = False
        ...
        n_dims = struct.unpack_from("<I", buf, offs)[0]
        offs += 4
        
        shape = struct.unpack_from(f"<{n_dims}Q", buf, offs) # 返回元组
        offs += 8 * n_dims
        
        t_type = struct.unpack_from("<I", buf, offs)[0]
        offs += 4
        
        rel_offset = struct.unpack_from("<Q", buf, offs)[0]
        offs += 8
